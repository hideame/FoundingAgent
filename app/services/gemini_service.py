...
class GeminiService:
    ...
    async def start_chat_session(
        ...
        3. `[[CONTENT_START]]`～`[[CONTENT_END]]`: 承認されたコンテンツを囲みます。ユーザーがOKした後に使用してください。
        4. `[[COMPLETED:task_id]]`: タスク完了を通知します。ユーザーがOKした後に使用してください。
        ...
        3. **タスク完了の通知（最重要・絶対必須）**:
             - ユーザーから「この内容でOKです」「OKです」「OK」「承認します」などの承認メッセージを受け取った場合、**例外なく必ず**以下の形式で応答してください：
               ...
               **必須の応答形式:**
               ...
               [[CONTENT_START]]
               【承認されたドラフト内容をそのまま再掲。改行もそのまま保持】
               [[CONTENT_END]]
               [[COMPLETED:タスクID]]

               それでは次に、【次の項目名】についてお伺いします。【次の質問】
               ```

             - **重要1**: 承認されたドラフト内容を `[[CONTENT_START]]` と `[[CONTENT_END]]` で囲んで必ず再掲してください。これはシステムがデータベースに保存するために必須です。
             - **重要2**: コンテンツ内の改行や空白はそのまま保持してください。
             - **重要3**: `[[CONTENT_END]]` の直後に改行を入れて `[[COMPLETED:タスクID]]` を記述してください。
             - **重要4**: `[[COMPLETED:タスクID]]` マーカーは絶対必須です。承認メッセージを受け取ったのにマーカーを出力しないことは禁止です。
             - **重要5**: `[[COMPLETED:タスクID]]` には、**今完了した項目のID**を指定してください。次の項目のIDではありません。
             - **具体例**: 「創業の動機」が完了した場合：
               ```
               ありがとうございます。
