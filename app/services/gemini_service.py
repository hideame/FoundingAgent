...
class GeminiService:
    ...
    async def start_chat_session(
        ...
        6. **経営者の略歴のドラフト（絶対厳守）**:
           ...
           - 日付の形式を「YYYY年M月：」形式に統一する（例: 2000年4月 → 2000年4月：）
           ...
           - 西暦を和暦に変換すること（システムが自動的に処理します）

           【正しいドラフト例】
           ユーザー入力:「2000年4月 A社入社（〇〇に従事）」
           正しいドラフト:「2000年4月：A社入社（〇〇に従事）」
           誤ったドラフト:「平成12年4月：A社入社（〇〇に従事）」（和暦に変換→禁止）
           誤ったドラフト:「2000年4月～2016年3月：A社入社（〇〇に従事）」（終了年月を追加→禁止）
           誤ったドラフト:「2000年4月～：A社入社（〇〇に従事）」（「～」を追加→禁止）

        7. **取引先情報のドラフト（絶対厳守）**:
           「5. 取引先・取引関係等」のドラフト作成時は、ユーザーが提供した情報をすべて省略せずに含めてください。

           【必ず含めるべき情報】
           - 取引先名（会社名・サービス名・個人等）
           - 所在地（都市名、国名、地域名など、ユーザーが提供したすべての地理情報）
           - シェア（%） - 例: 「シェア100%」
           - 掛取引の割合 - 例: 「掛取引100%」「掛取引50%」「即金」（現金の場合）
           - 回収・支払条件 - 例: 「月末締め翌月10日払い」「15日締め翌30日回収」「即金」

           【出力形式の重要ルール】
           - 各取引先の情報は1行にまとめて記載してください
           - 掛取引と回収支払条件は必ず数値や具体的な日付を含めてください
           - 「即金」の場合は「掛取引0% 即金」のように明記してください
           - 締日と支払日は「XX日締めYY日払い」のように続けて記載してください

           【絶対に禁止されること】
           - ユーザーが提供した所在地情報を省略・削除すること
           - 「個人」「CtoCサービス」といった理由で所在地を省略すること
           - ユーザーが書いていない情報を推測・追加すること
           - 掛取引の割合や回収支払条件を省略すること

           【正しいドラフト例】
           例1（即金の場合）:
           ユーザー入力:「販売先：個人 首都ハボロネ シェア100％ 即金」
           正しいドラフト:「販売先：個人（首都ハボロネ） シェア100% 掛取引0% 即金」

           例2（掛取引の場合）:
           ユーザー入力:「仕入先：クラスメソッド 東京都港区 シェア100% 掛取引100% 月末締め翌月10日払い」
           正しいドラフト:「仕入先：クラスメソッド（東京都港区） シェア100% 掛取引100% 月末締め翌月10日払い」

           誤ったドラフト:「販売先：個人（CtoCサービスのため） シェア100％ 即金」（所在地を削除→禁止）
           誤ったドラフト:「仕入先：クラスメソッド（東京都港区） シェア100%」（掛取引・回収支払条件を省略→禁止）
