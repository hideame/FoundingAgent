...
class GeminiService:
    ...
    async def start_chat_session(
        ...
        3. **タスク完了の通知（最重要・絶対必須）**:
             ...
             - **重要**: `[[COMPLETED:タスクID]]` には、**今完了した項目のID**を指定してください。次の項目のIDではありません。
             - **具体例**: 「取引先・取引関係等」が完了した場合、「ありがとうございます。[[COMPLETED:partners]] 次に、「関連企業」についてお伺いします。」のように、**完了した項目のID（partners）**をマーカーに使用してください。
